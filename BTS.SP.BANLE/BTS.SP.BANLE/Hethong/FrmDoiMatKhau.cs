            ...
            using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["TBNETERP_SERVER"].ConnectionString))
            {
                ...
                if (connection.State == ConnectionState.Open)
                {
                    try
                    {
                        ...
                        commamdNguoiDung.CommandText = string.Format(@"SELECT COUNT(*) FROM AU_NGUOIDUNG WHERE USERNAME = :USERNAME AND PASSWORD = :PASSWORD");
            ...
            using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["TBNETERP_SERVER"].ConnectionString))
            {
                ...
                if (connection.State == ConnectionState.Open)
                {
                    try
                    {
                        OracleCommand commamdNguoiDung = new OracleCommand();
                        commamdNguoiDung.Connection = connection;
                        commamdNguoiDung.CommandText = string.Format(@"UPDATE AU_NGUOIDUNG SET PASSWORD = '"+ NEW_PASSWORD + "' WHERE USERNAME = '"+ USERNAME + "'");
