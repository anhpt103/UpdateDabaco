...
IF P_USE_TIMKIEM_ALL = 1 THEN
    BEGIN
          ...
          IF SUBSTR(P_KEYSEARCH,0,2) = 'KH' THEN
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO MÃ KHÁCH HÀNG');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE '|| P_WHERE ||' UPPER(MAKH) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' AND UNITCODE = '''||P_UNITCODE||''' ';
          -- TÌM KIẾM THEO TÊN KHÁCH HÀNG
          ELSIF IS_CONTAIN_UNITCODE = 'X' THEN
           DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO TÊN KHÁCH HÀNG');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' UPPER(TENKH) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' AND UNITCODE = '''||P_UNITCODE||''' ';
          ...
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO SỐ ĐIỆN THOẠI');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' UPPER(UNITCODE) = '''||UPPER(P_UNITCODE)||''' AND (UPPER(DIENTHOAI) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'') ';
          ...
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO SỐ CHỨNG MINH THƯ NHÂN DÂN');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' (UPPER(CMTND) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' OR UPPER(MATHE) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' ) AND UNITCODE = '''||P_UNITCODE||''' ';
           -- TÌM KIẾM THEO SỐ ĐIỂM  
          ELSIF TEXT_IS_NUMBER = 1 AND LENGTH(P_KEYSEARCH) < 9  THEN
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO SỐ ĐIỂM');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' UPPER(SODIEM) = '||UPPER(P_KEYSEARCH)||' AND UNITCODE = '''||P_UNITCODE||''' ';
          ELSE
            DBMS_OUTPUT.PUT_LINE('TÌM KIẾM MẶC ĐỊNH');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' ( UPPER(MAKH) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' 
                            OR UPPER(DIENTHOAI) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' OR UPPER(CMTND) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' 
                            OR UPPER(TENKH) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' OR UPPER(MATHE) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' ) 
...
ELSE
    BEGIN
          ...
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO MÃ THẺ');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE '|| P_WHERE ||' UPPER(MATHE) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' AND UNITCODE = '''||P_UNITCODE||''' ';
          -- TÌM KIẾM THEO TÊN KHÁCH HÀNG
          ...
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO TÊN KHÁCH HÀNG');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' UNITCODE = '''||P_UNITCODE||''' AND (UPPER(TENKH) LIKE N''%'||UPPER(P_KEYSEARCH)||'%'') ';
          -- TÌM KIẾM THEO SỐ ĐIỆN THOẠI
          ELSIF P_DIEUKIEN_TIMKIEM = 3 THEN
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO SỐ ĐIỆN THOẠI');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' UNITCODE = '''||P_UNITCODE||''' AND (UPPER(DIENTHOAI) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'') ';
          -- TÌM KIẾM THEO SỐ CHỨNG MINH THƯ NHÂN DÂN
          ELSIF P_DIEUKIEN_TIMKIEM = 4 THEN
          DBMS_OUTPUT.PUT_LINE('TÌM KIẾM THEO SỐ CHỨNG MINH THƯ NHÂN DÂN');
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' UPPER(CMTND) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' AND UNITCODE = '''||P_UNITCODE||''' ';
           -- TÌM KIẾM THEO SỐ ĐIỂM 
          ...
          ELSE
           ...
           QUERY_SELECT := 'SELECT MAKH,TENKH,DIACHI,DIENTHOAI,CMTND,EMAIL,SODIEM,TONGTIEN,NGAYCAPTHE,NGAYHETHAN,NGAYSINH,UNITCODE,HANGKHACHHANG,HANGKHACHHANGCU 
                            FROM DM_KHACHHANG 
                            WHERE  '|| P_WHERE ||' ( UPPER(MAKH) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' OR UPPER(DIENTHOAI) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' 
                            OR UPPER(CMTND) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' OR UPPER(TENKH) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' OR UPPER(MATHE) LIKE ''%'||UPPER(P_KEYSEARCH)||'%'' ) 
                            AND UNITCODE = '''||P_UNITCODE||''' ';
        END IF;
    END;
END IF;
