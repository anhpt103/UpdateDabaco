...
* View: BTS.SP.MART/views/htdm/AuNhomQuyenChucNang
* Menu: Danh mục-> Danh mục nhóm quyền
*/
define(['ui-bootstrap', '/BTS.SP.MART/controllers/auth/AuMenu.js'], function () {
    'use strict';
    var app = angular.module('AuNhomQuyenChucNangModule', ['ui.bootstrap', 'AuMenuModule']);
    app.factory('AuNhomQuyenChucNangChucNangService', ['$http', 'configService', function ($http, configService) {
        var serviceUrl = configService.rootUrlWebApi + '/Authorize/AuNhomQuyenChucNang';
        var result = {
    ...
    /* controller list */
    app.controller('AuNhomQuyenChucNangConfigCtrl', ['$scope', '$location', '$http', '$uibModalInstance', 'configService', 'AuNhomQuyenChucNangChucNangService', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'securityService', 'toaster', 'AuMenuService', 'targetData',
        function ($scope, $location, $http, $uibModalInstance, configService, service, tempDataService, $filter, $uibModal, $log, ngNotify, securityService, toaster, AuMenuService, targetData) {
                ...
                if (item.menuIdCha != '') {
                    ...
                    if (existParent && existParent.length === 1) {
                        ...
                        angular.forEach(existParent[0].children, function (item, key) {
                            var index = $scope.lstAdd.indexOf(item[0]);
                            $scope.lstAdd.splice(index, 1);
                            $scope.data_add = treeify($scope.lstAdd).children;
                            $scope.lstChucNang.push(item);
                        });
            ...
            $scope.deSelectChucNangAdd = function (item) {
                console.log(item);
                var existParent = $filter('filter')($scope.lstEdit, { machucnang: item.machucnang }, true);
                if (existParent && existParent.length === 1) {
                    var index = $scope.lstEdit.indexOf(existParent[0]);
                    $scope.lstEdit.splice(index, 1);
                    var indexParent = -1;
                    for (var i = 0; i < $scope.data_update.length; i++) {
                        if (existParent[0].machucnang === $scope.data_update[i].machucnang) {
                            indexParent = i;
                        }
                    }
                    $scope.data_update.splice(indexParent, 1);
                    angular.forEach(existParent[0].children, function (item, key) {
                        var index = $scope.lstEdit.indexOf(item[0]);
                        $scope.lstEdit.splice(index, 1);
                        $scope.data_update = treeify($scope.lstEdit).children;
                    });
                    $scope.lstDelete.push({
                        ID: existParent[0].id,
                        username: existParent[0].username,
                        machucnang: existParent[0].machucnang
                    });
                }
            }
            ...
            $scope.save = function () {
                ...
                // $scope.lstAdd.forEach(function (obj) {
                // if (obj.menuIdCha == "") {
                // var index = $scope.lstAdd.indexOf(obj);
                // $scope.lstAdd.splice(index, 1);
                // }
                // });
                var obj = {
