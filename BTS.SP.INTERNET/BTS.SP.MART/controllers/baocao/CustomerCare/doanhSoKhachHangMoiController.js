...
* Menu: Nghiệp vụ-> Chăm sóc khách hàng doanh số khách hàng mới
*/
define(['ui-bootstrap', '/BTS.SP.MART/controllers/auth/AuthController.js', '/BTS.SP.MART/controllers/htdm/periodController.js', '/BTS.SP.MART/controllers/htdm/merchandiseController.js', '/BTS.SP.MART/controllers/htdm/customerController.js', '/BTS.SP.MART/controllers/htdm/merchandiseTypeController.js', '/BTS.SP.MART/controllers/htdm/nhomVatTuController.js', '/BTS.SP.MART/controllers/htdm/supplierController.js', '/BTS.SP.MART/controllers/htdm/wareHouseController.js', '/BTS.SP.MART/controllers/htdm/packagingController.js', '/BTS.SP.MART/controllers/htdm/taxController.js', '/BTS.SP.MART/controllers/htdm/donViTinhController.js'], function () {
    'use strict';
    var app = angular.module('doanhSoKhachHangMoiModule', ['ui.bootstrap', 'authModule', 'periodModule', 'merchandiseModule', 'customerModule', 'merchandiseTypeModule', 'nhomVatTuModule', 'supplierModule', 'wareHouseModule', 'packagingModule', 'taxModule', 'donViTinhModule']);
    app.factory('doanhSoKhachHangMoiService', ['$http', 'configService', function ($http, configService) {
        var serviceUrl = configService.rootUrlWebApi + '/Ac/DoanhSoMoi';
        ...
        var result = {
            robot: calc,
            postCreateReportDoanhSoMoi: function (data, callback) {
                $http.post(serviceUrl + '/ReportDoanhSoMoi', data).success(callback);
            },
            postCreateReportDoanhSoMoiDetails: function (data, callback) {
                $http.post(serviceUrl + '/ReportDoanhSoMoiDetails', data).success(callback);
            },
            getUnitUsers: function (callback) {
                $http.get(rootUrl + '/api/Authorize/AuDonVi/GetSelectDataByUnitCode').success(callback);
            },
            getCustomers: function (callback) {
                $http.get(rootUrl + '/api/MD/Customer/GetSelectData').success(callback);
            },
    ...
    app.controller('doanhSoKhachHangMoiController', [
        ...
        function ($scope, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, ngNotify, securityService, $rootScope, toaster, servicePeriod, serviceMerchandise, serviceCustomer, serviceMerchandiseType, serviceNhomVatTu, serviceSupplier, serviceWareHouse, servicePackaging, serviceTax, serviceDonViTinh, $state) {
            ...
            //load dữ liệu
            function filterData() {
                $scope.tagWareHouses = serviceWareHouse.getSelectData();
                service.getUnitUsers(function (response) {
                    $scope.unitUsers = response;
                });
                service.getCustomers(function (response) {
                    $scope.customers = response;
                });
                service.getNewParameter(function (response) {
                    console.log(response);
                    $scope.target = response;
                    $scope.options.maxDate = response.maxDate;
                });
                $scope.tagCustomers = serviceCustomer.getSelectData();
            };
            //end

            //check quyền truy cập
            function loadAccessList() {
                securityService.getAccessList('doanhSoKhachHangMoi').then(function (successRes) {
            ...
            $scope.doSearch = function () {
                ...
                service.postNotBuyCustomer($scope.target, function (response) {
                    if (response.status) {
            ...
            $scope.report = function () {
                ...
                $state.go('doanhSoKhachHangMoiReport', { obj: $scope.target });
    ...
    /* controller reportDoanhSoKhachHangMoiController */
    app.controller('reportDoanhSoKhachHangMoiController', ['$scope', '$uibModalInstance', '$location', '$http', 'configService', 'doanhSoKhachHangMoiService', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', '$rootScope', 'userService', 'merchandiseService', 'customerService', 'merchandiseTypeService', 'nhomVatTuService', 'supplierService', 'wareHouseService', 'packagingService', 'taxService', 'donViTinhService', 'periodService', '$state',
        function ($scope, $uibModalInstance, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, ngNotify, $rootScope, serviceAuthUser, serviceMerchandise, serviceCustomer, serviceMerchandiseType, serviceNhomVatTu, serviceSupplier, serviceWareHouse, servicePackaging, serviceTax, serviceDonViTinh, servicePeriod, $state) {
            ...
            $scope.goIndex = function () {
                $state.go('doanhSoKhachHangMoi');
            };
            if (para) {
                service.postCreateReportDoanhSoMoiDetails(para, function (response) {
                    if (response != null) {
