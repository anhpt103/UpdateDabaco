/*  
* Người tạo : HuyNQ
* View: BTS.SP.MART/views/nv/nvCongNoKhachHang
...
define(['ui-bootstrap', '/BTS.SP.MART/controllers/auth/AuthController.js', '/BTS.SP.MART/controllers/htdm/periodController.js', '/BTS.SP.MART/controllers/htdm/merchandiseController.js', '/BTS.SP.MART/controllers/htdm/customerController.js', '/BTS.SP.MART/controllers/htdm/merchandiseTypeController.js', '/BTS.SP.MART/controllers/htdm/nhomVatTuController.js', '/BTS.SP.MART/controllers/htdm/supplierController.js', '/BTS.SP.MART/controllers/htdm/wareHouseController.js', '/BTS.SP.MART/controllers/htdm/packagingController.js', '/BTS.SP.MART/controllers/htdm/taxController.js', '/BTS.SP.MART/controllers/htdm/donViTinhController.js'], function () {
    'use strict';
    var app = angular.module('nvCongNoKhachHangModule', ['ui.bootstrap', 'authModule', 'periodModule', 'merchandiseModule', 'customerModule', 'merchandiseTypeModule', 'nhomVatTuModule', 'supplierModule', 'wareHouseModule', 'packagingModule', 'taxModule', 'donViTinhModule']);
    app.factory('nvCongNoKhachHangService', ['$http', 'configService', function ($http, configService) {
        ...
        var result = {
            ...
            getAmmountCustomerBorrowed: function (data) {
                return $http.get(serviceUrl + '/GetAmmountCustomerBorrowed/' + data);
            },
    ...
    /* controller list */
    app.controller('nvCongNoKhachHangController', [
        '$scope', '$location', '$http', 'configService', 'nvCongNoKhachHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'securityService', '$rootScope', 'toaster', 'customerService', 'supplierService',
        function ($scope, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, ngNotify, securityService, $rootScope, toaster, serviceCustomer, serviceSupplier) {
            ...
            function loadAccessList() {
                securityService.getAccessList('nvCongNoKhachHang').then(function (successRes) {
            ...
            $scope.title = function () { return 'Phiếu công nợ thu khách hàng'; };
            ...
            $scope.create = function () {
                var modalInstance = $uibModal.open({
                    backdrop: 'static',
                    size: 'md',
                    templateUrl: configService.buildUrl('nv/NvCongNoKhachHang', 'add'),
                    controller: 'nvCongNoKhachHangCreateController',
            ...
            $scope.update = function (target) {
                var modalInstance = $uibModal.open({
                    backdrop: 'static',
                    templateUrl: configService.buildUrl('nv/NvCongNoKhachHang', 'update'),
                    controller: 'nvCongNoKhachHangEditController',
                    windowClass: 'app-modal-window',
                    resolve: {
                        targetData: function () {
                            return target;
            ...
            $scope.details = function (target) {
                var modalInstance = $uibModal.open({
                    backdrop: 'static',
                    templateUrl: configService.buildUrl('nv/NvCongNoKhachHang', 'details'),
                    controller: 'nvCongNoKhachHangDetailsController',
                    windowClass: 'app-modal-window',
                    resolve: {
                        targetData: function () {
                            return target;
            ...
            $scope.deleteItem = function (event, target) {
                var modalInstance = $uibModal.open({
                    backdrop: 'static',
                    templateUrl: configService.buildUrl('nv/NvCongNoKhachHang', 'delete'),
                    controller: 'nvCongNoKhachHangDeleteController',
                    resolve: {
                        targetData: function () {
                            return target;
                        }
    ...
    /* controller addNew */
    app.controller('nvCongNoKhachHangCreateController', ['$scope', '$uibModalInstance', '$location', '$http', 'configService', 'nvCongNoKhachHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'ngNotify', 'customerService', 'supplierService',
        function ($scope, $uibModalInstance, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, ngNotify, serviceCustomer, serviceSupplier) {
            ...
            $scope.target = {};
            $scope.isLoading = false;
            $scope.title = function () { return 'Thêm mới phiếu công nợ thu Khách hàng'; };
            ...
            $scope.selectedkhachHang = function (maKH) {
                service.getAmmountCustomerBorrowed(maKH).then(function (response) {
    ...
    /* controller Edit */
    app.controller('nvCongNoKhachHangEditController', ['$scope', '$uibModalInstance', '$location', '$http', 'configService', 'nvCongNoKhachHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'targetData', 'ngNotify',
        function ($scope, $uibModalInstance, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, targetData, ngNotify) {
            ...
            $scope.data = [];
            $scope.isLoading = false;
            $scope.title = function () { return 'Sửa phiếu công nợ thu Khách hàng'; };
            $scope.formatLabel = function (paraValue, moduleName) {
                var data = $filter('filter')($scope.tempData(moduleName), { value: paraValue }, true);
                if (data && data.length === 1) {
                    return data[0].text;
    ...
    /* controller Details */
    app.controller('nvCongNoKhachHangDetailsController', ['$scope', '$uibModalInstance', '$location', '$http', 'configService', 'nvCongNoKhachHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'targetData', 'ngNotify',
        function ($scope, $uibModalInstance, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, targetData, ngNotify) {
            $scope.config = angular.copy(configService);
            $scope.paged = angular.copy(configService.pageDefault);
            $scope.tempData = tempDataService.tempData;
    ...
    /* controller delete */
    app.controller('nvCongNoKhachHangDeleteController', ['$scope', '$uibModalInstance', '$location', '$http', 'configService', 'nvCongNoKhachHangService', 'tempDataService', '$filter', '$uibModal', '$log', 'targetData', 'ngNotify',
        function ($scope, $uibModalInstance, $location, $http, configService, service, tempDataService, $filter, $uibModal, $log, targetData, ngNotify) {
            $scope.config = angular.copy(configService);
            $scope.isLoading = false;
